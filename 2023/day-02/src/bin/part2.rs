...
fn get_cube_sum(games: &Games) -> GameSet {
    let mut game_sum = GameSet::new();
    for game_set in &games.game_sets {
        if game_set.red > game_sum.red {
            game_sum.red = game_set.red
        }
        if game_set.green > game_sum.green {
            game_sum.green = game_set.green
        }
        if game_set.blue > game_sum.blue {
            game_sum.blue = game_set.blue
        }
    }
...
fn part1(input: &str) -> usize {
    ...
    for line in input.lines() {
        ...
        let game_set_sum = get_cube_sum(&games);
        println!("{:?}", game_set_sum);
        game_numbers += game_set_sum.red * game_set_sum.green * game_set_sum.blue
    }
    game_numbers
}
